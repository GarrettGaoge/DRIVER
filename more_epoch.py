...
# python3 more_epoch.py --test_epoch 69
# user average pooling to aggregate the next room embeddings

os.environ['CUDA_VISIBLE_DEVICES'] = '0'
arg = args.parse_args()
# DEFINE MODEL TYPE
model_type = 'room_next_4_%f'%arg.lr
# LOAD DATA
...
model = DEPIE(num_users, num_items, arg.embd_size, arg.gnn_layers).cuda()
MSELoss = nn.MSELoss()
# OPTIM
lr = arg.lr
...
model, opt, user_embd, item_embd, item_next_embd, _\
    = load_model(model, opt, arg.test_epoch,arg, model_type)
# SET FILE FOR TENSORBOARD
board_file = './boards/'+model_type+'_more_2'+'/'
...
with trange(arg.test_epoch, arg.test_epoch+20) as epochs:
    for epoch in epochs:
        last_true = dict()
                ...
                if (timestamp - tbatch_start_time > tbatch_timespan) | (train_sample == train_end_idx-1):
                    ...
                    with trange(len(ut.current_tbatches_user)) as batches:
                        for batch in batches:
                            ...
                            item_state = model.aggregate_function(item_embd.detach(),
                                                                  user_embd.detach(),
                            ...
                            del item_embd_input
                            del user_embd_input
                            del item_embd_out_pos
                            del user_embd_out_pos
                            del predicted_item_embd
                            del user_projected_embd
                            del tbatch_userids
                            del tbatch_itemids
                    # RECORD THE LOSS
